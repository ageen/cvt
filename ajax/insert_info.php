...
switch($type):
	case "edu":
		...
		$sql = $idb->query_insert($feed, "info_education");
		if($idb->query_ex($sql)){
...
<?php
	...
	case "job":
		...
		$sql = $idb->query_insert($feed, "info_job");
		if($idb->query_ex($sql)){
...
<button onClick="ajax_delete(<?php echo $v["id"];?>,'job',4);">删除</button>
